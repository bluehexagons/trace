...
const resolveStdlibCategories = (
  opt: TraceStdlibOptions | boolean | undefined,
): Set<TraceStdlibCategory> => {
...
const closeStatement = (
  ...
  if (f.arrayWrite) {
    ...
    if (arr !== undefined && idx >= 1 && idx < arr.length) {
      ...
      throw new Error(
        `Runtime error: index ${idx} out of bounds for array "${f.writeArray}" (size ${arr[0]})`,
      );
    }
    f.setVar = '';
    f.writeArray = '';
...
const resolveCallable = (
  arg: Trace | undefined,
  ctx: StdlibContext,
): { trace: Trace; params: string[] } | null => {
  ...
  if (
    t.kind === TokenKind.aFunction ||
    t.kind === TokenKind.function ||
    t.kind === TokenKind.aLambda ||
    t.kind === TokenKind.lambda
  ) {
    if (t.callable === undefined) {
      ...
      const body = isFn
        ...
        : t.string.substring(
            t.string.indexOf('>') + 1,
            t.string.endsWith(';') ? t.string.length - 1 : t.string.length,
          );
...
export class Trace {
            ...
            if (t.string.startsWith('&') && t.string.length > 1) {
              ...
              if (/[0-9]/.test(t.string[1])) {
                this.logger(
                  ...
                  s !== null ? s[parseInt(t.string.substring(1), 10)] : undefined,
                );
              } else {
            ...
            if (functions.has(fn)) {
              ...
              for (let i = 0; i < ms.callParams.length; i++) {
                ...
                if (
                  ...
                  argTrace.tokens[0].kind === TokenKind.variable
                ) {
  ...
  runWithOptions(options: TraceRunOptions = {}): TraceRunResult {
    ...
    const functions =
      options.memory?.functions ?? (options.persist ? null : new Map<string, Trace>());
    const arrays =
      options.memory?.arrays ?? (options.persist ? null : new Map<string, Float64Array>());
    const startedAt = now();
