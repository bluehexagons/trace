...
describe('arrays', () => {
  ...
  it('writes multiple elements independently', () => {
    expect(
      runTrace('arr = [3]; arr[1] = 10; arr[2] = 20; arr[3] = 30; arr[1] + arr[2] + arr[3]'),
    ).toBe(60);
  });
  ...
  it('works in function bodies', () => {
    expect(
      runTrace('arr = [3]; fill()=>{arr[1]=1;arr[2]=2;arr[3]=3}; fill(); arr[1]+arr[2]+arr[3]'),
    ).toBe(6);
  });

  it('loop-fills an array and sums it', () => {
    const script =
      'arr = [5]; i = 1; i <= 5 ? ()=>{arr[i] = i; i++ <= 5 ? () : 0}; s = 0; j = 1; j <= 5 ? ()=>{s += arr[j]; j++ <= 5 ? () : s}';
...
describe('stdlib loops', () => {
  ...
  it('supports nested loops', () => {
    expect(
      runTrace('t = 0; i = 0; while(i < 3, { j = 0; while(j < 4, { t++; j++ }); i++ }); t'),
    ).toBe(12);
  });
...
describe('stdlib arrays', () => {
  it('foreach visits every element', () => {
    expect(
      runTrace('arr = [3]; arr[1]=1; arr[2]=2; arr[3]=3; t=0; foreach(arr, (x) => t += x); t'),
    ).toBe(6);
  });
  ...
  it('mapmut mutates the array in place', () => {
    expect(
      runTrace(
        'arr = [3]; arr[1]=1; arr[2]=2; arr[3]=3; mapmut(arr, (x) => x * 10); arr[1]+arr[2]+arr[3]',
      ),
    ).toBe(60);
  });
  ...
  it('map creates a new array and leaves the source intact', () => {
    expect(
      runTrace(
        'arr = [2]; arr[1]=4; arr[2]=5; out = map(arr, (x) => x * 10); arr[1]*1000+arr[2]*100+out[1]+out[2]',
      ),
    ).toBe(4590);
  });
  ...
  it('map result is assignable and indexable', () => {
    expect(
      runTrace(
        'arr = [3]; arr[1]=1; arr[2]=2; arr[3]=3; double(x)=>{x*2}; out = map(arr, double); out[1]+out[2]+out[3]',
      ),
    ).toBe(12);
  });
  ...
  it('reduce folds elements with an accumulator', () => {
    expect(
      runTrace('arr = [3]; arr[1]=10; arr[2]=20; arr[3]=30; reduce(arr, (a, b) => a + b)'),
    ).toBe(60);
  });

  it('reduce accepts an initial value as the third arg', () => {
    expect(
      runTrace('arr = [3]; arr[1]=1; arr[2]=2; arr[3]=3; reduce(arr, (a, b) => a + b, 100)'),
    ).toBe(106);
  });
  ...
  it('sort sorts ascending by default', () => {
    expect(
      runTrace(
        'arr = [4]; arr[1]=3; arr[2]=1; arr[3]=4; arr[4]=2; sort(arr); arr[1]*1000+arr[2]*100+arr[3]*10+arr[4]',
      ),
    ).toBe(1234);
  });
  ...
  it('sort accepts a custom comparator', () => {
    expect(
      runTrace(
        'arr = [3]; arr[1]=1; arr[2]=3; arr[3]=2; sort(arr, (a, b) => b - a); arr[1]*100+arr[2]*10+arr[3]',
      ),
    ).toBe(321);
  });
  ...
  it('find returns the 1-based index of the first match (0 when none)', () => {
    expect(runTrace('arr = [3]; arr[1]=5; arr[2]=10; arr[3]=15; find(arr, (x) => x == 10)')).toBe(
      2,
    );
    expect(runTrace('arr = [3]; arr[1]=5; arr[2]=10; arr[3]=15; find(arr, (x) => x == 99)')).toBe(
      0,
    );
  });
});
