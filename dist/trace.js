...
for (const t of [24, 25, 26, 27, 28, 29]) {
    opLevels.set(t, 1);
}
class StackFrame {
...
class Trace {
    constructor(body, tokens, params, stackSize) {
        ...
        this.logger = Trace.logger;
        this.errorLogger = Trace.errorLogger;
        this.lastRunTime = 0;
        this.vars = null;
        this.functions = null;
        ...
        for (;;) {
            ...
            if (kind === 48) {
                ...
                if (stringLeft.length === 0) {
                    while (groupLevel > 0) {
                        groupLevel--;
                        tokens.splice(tokens.length, 0, { kind: 10, value: NaN, string: ')' });
                    }
                    return new Trace(preprocessed, tokens, params, stackSize);
                }
                continue;
            ...
            if (findOperator && kind !== 10 && kind !== 46 && kind !== 47 && kind !== 44 && kind !== 45) {
                const opLevel = opLevels.has(kind) ? opLevels.get(kind) : 0;
                while (groupLevel < opLevel) {
                    groupLevel++;
                    tokens.splice(loi[loi.length - 1], 0, { kind: 9, value: NaN, string: '(' });
                }
                while (groupLevel > opLevel) {
                    groupLevel--;
                    tokens.splice(tokens.length, 0, { kind: 10, value: NaN, string: ')' });
    ...
    run(args = [], variables = null, vars = null, functions = null, rand = Math.random, executionLimit = 1000, executionStart = perf_hooks_1.performance.now()) {
        const frames = [];
        let split = [];
        let fn = '';
        let script = '';
        ...
        callStack: while (frames.length > 0) {
            ...
            for (; f.i < f.tokens.length; f.i++) {
                ...
                let val = null;
                if (perf_hooks_1.performance.now() - executionStart > executionLimit) {
                    this.errorLogger('Trace timed out');
                    this.lastRunTime = perf_hooks_1.performance.now() - executionStart;
                    return 0;
                }
                switch (t.kind) {
                    case 48:
...
exports.Trace = Trace;
Trace.logger = console.log;
Trace.errorLogger = console.error;
exports.runTrace = (script, ...args) => {
